...
class TestActorCritic(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.mlp_actor = TD3ActorMLP(
            (cls.num_observations,128,128,1)
        )
        cls.lstm_actor = TD3ActorLSTM(
            (cls.num_observations,128,1)
        )
        cls.mlp_critic = TD3CriticMLP((cls.num_observations+1,128,128,1))
        cls.lstm_critic = TD3CriticLSTM((cls.num_observations+1,128,1))
        cls.states = torch.rand(
            (cls.num_envs,cls.num_observations),device = cls.device
        )
        cls.actions = torch.rand((cls.num_envs,cls.num_observations),device = cls.device)

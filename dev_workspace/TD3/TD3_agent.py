...
class TD3Agent(BaseObject):
    ...
    def target_networks_step(self, next_states: torch.Tensor):
        target_actions = self.actor_target.take_noised_action(next_states)
        state_action_values_target_1 = self.critic_target_1.forward(
            torch.cat([next_states, target_actions], dim=1)
        ).detach()
        state_action_values_target_2 = self.critic_target_2.forward(
            torch.cat([next_states, target_actions], dim=1)
        ).detach()
        return (
            target_actions,
    ...
    def train(self, current_states: torch.Tensor) -> int:
        ...
        for _ in range(self.num_epochs):
            ...
            batch_state_action_values_target_1 = batch_dict[
                "state_action_values_target_1"
            ]
            batch_state_action_values_target_2 = batch_dict[
                "state_action_values_target_2"
            ]
            ...
            for iteration_num, mini_batch_indices in enumerate(mini_batch_list):
                ...
                mini_batch_state_action_values_1 = batch_state_action_values_1[
                    mini_batch_indices, :
                ]
                mini_batch_state_action_values_2 = batch_state_action_values_2[
                    mini_batch_indices, :
                ]
                mini_batch_state_action_values_target_1 = (
                    batch_state_action_values_target_1[mini_batch_indices, :]
                )
                mini_batch_state_action_values_target_2 = (
                    batch_state_action_values_target_2[mini_batch_indices, :]
                )

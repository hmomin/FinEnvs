...
class Actor(nn.Module):
    def __init__(self, learning_rate: float) -> None:
        ...
        self.device = device
        self.fully_connected_1 = nn.Linear(3, 128, device=self.device)
        ...
        self.log_alpha = torch.tensor(np.log(starting_alpha), device=self.device)
...
class Critic(nn.Module):
    def __init__(self, learning_rate: float) -> None:
        ...
        self.device = device
        self.fully_connected_state = nn.Linear(3, 64, device=self.device)
...
def calc_target(
    ...
    with torch.no_grad():
        ...
        not_dones = 1 - dones
        target = rewards + gamma * not_dones * (min_q_values + entropy)
    return target
...
def main():
    ...
    for episode_num in range(1000):
        ...
        while not done:
            tensor_state = torch.tensor(state, device=device).unsqueeze(0).float()
            action = actor.forward(tensor_state)[0].detach()
            next_state, reward, done, info = env.step([2.0 * action.item()])
            tensor_next_state = (
                torch.tensor(next_state, device=device).unsqueeze(0).float()
            )
            tensor_reward = torch.tensor(reward / 10, device=device)
            tensor_done = torch.tensor(done, device=device)
            buffer.store(
                tensor_state,
                action,
                tensor_reward,

...
def main():
    ...
    buffer = ReplayBuffer()
    proxy_buffer = Buffer(max_size=max_size, device_id=-1)
    ...
    for episode_num in range(1000):
        ...
        while not done:
            ...
            tensor_next_state = torch.from_numpy(next_state).float().unsqueeze(0)
            tensor_reward = torch.tensor(reward / 10.0, dtype=torch.float)
            tensor_done = torch.tensor(done, dtype=torch.int)
            proxy_buffer.store(
                tensor_state,
                action.float(),
                tensor_reward,
            ...
            if proxy_buffer.size() > minibatch_size:
                # mini_batch = buffer.sample(minibatch_size)
                mini_batch_dict = proxy_buffer.get_mini_batch(minibatch_size)
                mini_batch = (
                    mini_batch_dict["states"],
                    mini_batch_dict["actions"],
                    mini_batch_dict["rewards"],
                    mini_batch_dict["next_states"],
                    mini_batch_dict["dones"],
                )

...
class LSTMNetwork(GenericNetwork):
    def __init__(
        ...
        super(LSTMNetwork, self).__init__(device_id=device_id)
    ...
    def create_layers(self, shape: tuple, output_activation) -> None:
        ...
        self.num_layers = 2
        self.lstm = nn.LSTM(
            1, self.hidden_size, self.num_layers, batch_first=True, device=self.device
        )
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        lstm_inputs = torch.unsqueeze(inputs, dim=-1)
        # if this is a single batch, we may still need an extra dimension...
        if len(lstm_inputs.shape) == 2:
